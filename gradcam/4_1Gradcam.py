...
train_dir     = Path(r"C:\NEW_Aurora_jpg\Class_Random\val\yes")  ### ‼️‼️원본 이미지 경로
weights_path  = "best_efficientnetb1_yesno.pth"  # fine-tuned 가중치
out_folder    = Path("cam_overlays (val yes)")  ### ‼️‼️Grad cam 이미지 + npy 저장 경로 
raw_folder    = out_folder / "raw_cam"
out_folder.mkdir(exist_ok=True);  raw_folder.mkdir(exist_ok=True)
...
for img_path in img_paths:
    ...
    # 3) Grad-CAM (yes=1) ‼️‼️yes로 분류될 때 가장 큰 영향을 끼친 영역 
    targets = [ClassifierOutputTarget(1)]
    grayscale_cam = cam(inp, targets=targets)[0]          # H×W 0-1
    orig_h, orig_w = rgb.shape[:2]
    grayscale_cam = cv2.resize(grayscale_cam, (orig_w, orig_h))
    # 4) raw np파일 저장
    np.save(raw_folder / f"{name}_cam.npy", grayscale_cam)
    # 5) overlay png이미지 시각화 ‼️‼️나중에 제발 jpg로 바꿔라....
...
print(f"✅ 완료! overlay→ {out_folder}, raw heatmap→ {raw_folder}")
